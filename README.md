...
面向 OpenClaw 的高信噪比中文晨间新闻简报 skill。它从实时来源中整理 AI、科技、财经与国际新闻，按“事件”而非“文章”去重、核验关键事实，并产出约五分钟可读完的编辑式早报。
...
建议在 OpenClaw 中使用 `openai/gpt-5.6-sol` 与 `xhigh` 推理等级，并将托管搜索提供商固定为 Brave。模型和推理等级属于宿主运行时配置，不由 skill 自行修改。
...
不要把真实 API 密钥提交到本仓库或写入 skill 文件。
